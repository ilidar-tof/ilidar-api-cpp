		...
		if (ch == 'q' || ch == 'Q') {
			...
			for (int _i = 0; _i < ilidar->device_cnt; _i++) {
				...
				if (ilidar->device[_i].status.sensor_sn == ilidar->device[_i].info.sensor_sn) {
					...
					ilidar->device[_i].info.capture_shutter[1] = 40;
					ilidar->device[_i].info.capture_shutter[2] = 4;
					ilidar->device[_i].info.capture_shutter[3] = 2;
					ilidar->device[_i].info.capture_shutter[4] = 800;
					...
					printf("[MESSAGE] iTFS::LiDAR config packet was sent to D#%d.\n", _i);
				}
			}
		}
		else if (ch == 'w' || ch == 'W') {
			...
			for (int _i = 0; _i < ilidar->device_cnt; _i++) {
				// Check the info packet was received or not
				if (ilidar->device[_i].status.sensor_sn == ilidar->device[_i].info.sensor_sn) {
					/* The serial number is not matched */
